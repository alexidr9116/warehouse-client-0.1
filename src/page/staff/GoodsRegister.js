                    ...
                    {currentStep === 0 &&
                        <div>
                            ...
                            <div className="flex justify-center w-full mt-4 gap-1 ">
                                <button disabled={(!warehouse || !warehouse._id || !barcode || (barcode && barcode.length === 0))} className="btn   btn-info btn-outline mt-2 w-[100px]" onClick={() => setCurrentStep(1)} >{t('words.next')}</button>
                                <button disabled={(!warehouse || !warehouse._id || !barcode || (barcode && barcode.length === 0))} className={`${saving ? 'loading' : ''}  w-[100px] btn btn-info  mt-2  `} type = "submit" >{t('words.save')}</button>
                            </div>
                        </div>
                    }
                    {
                        currentStep === 1 && <div className="w-full flex-col max-w-xl">
                            ...
                            <div className="grid md:grid-cols-2 items-center justify-center  gap-4  p-4">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <label>Price (¥) </label>
                                    <input type="number" step={0.01} className="input input-bordered rounded"  {...register('priceY')} />
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label>Title</label>
                                    ...
                                    <label>Size (M³)</label>
                                    <input className="input input-bordered rounded" step={0.01} type= "number" {...register('size')} />
                                    <label>Type</label>
                                ...
                                <div className=" w-full mt-4 md:col-span-2 justify-center flex">
                                    ...
                                    <button className={`btn btn-info  w-full ${saving ? 'loading' : ''}`} type="submit">Register</button>
                                </div>
                            </div>
                        </div>
                    }
