...
export default function GoodsArrived() {
    ...
    const onSubmit = (data) => {
        ...
        if (warehouse && warehouse._id) {
            SEND_PUT_REQUEST(API_ADMIN.arrivedUb, { ...data, warehouseId: warehouse._id }).then(res => {
                setSaving(false);
                if (res.status === 200) {
    ...
    useEffect(() => {
        if (currentStep === 1 && warehouse && warehouse._id) {
            SEND_POST_REQUEST(API_CLIENT.getProductByBarcode, { barcode, warehouseId: warehouse._id }).then(res => {
                console.log(res);
                if (res.status === 200) {
                        ...
                        currentStep === 1 && <div className="w-full flex-col max-w-xl">
                            ...
                            <div className="grid md:grid-cols-2 items-center justify-center  gap-4  p-4">
                                <div className="flex flex-col gap-2">
                                    <label>Mobile(*)</label>
                                    <input type="number" className="input input-bordered rounded" required  {...register('mobile')} />
                                    <label>Weight(Kg) *</label>
                                    <input type="number" step={0.01} required className="input input-bordered rounded"  {...register('weight')}  />
                                    <label>Price (₮) *</label>
                                    <input type="number" step={0.01} className="input input-bordered rounded"  {...register('price')} required />
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <label>Size (M³)*</label>
                                    <input className="input input-bordered rounded" step={0.01} required type="number"  {...register('size')}  />
                                   
                                    <label>Price Y(¥) </label>
                                    <input type="number" step={0.01} className="input input-bordered rounded"  {...register('priceY')} disabled />
                                </div>
                                ...
                                <div className=" w-full mt-4 md:col-span-2 justify-center flex">
                                    <button className={`btn btn-info w-full ${saving ? 'loading' : ''}`} type="submit">Arrived UB</button>
                                </div>
                            </div>
                        </div>
                    }
